...
metadata:
  author: John Lawson
  url: go.johnlawson.com/aiprofitlab
  version: 1.0 (AIPL Edition)
---
# AIPL - Tutorial Writer
...
```markdown
# AIPL - How to [Achieve Specific Outcome]
...
**GATE: Do not proceed until required inputs are confirmed. If any are missing, ask the user directly — do not assume or substitute.**
...
# AIPL - How to Connect Stripe Payments to Your Shopify Store
...
# AIPL - How to Create a Welcome Email Sequence in Mailchimp
...
- **Skipping the Brief Phase** — Producing output without confirmed inputs leads to generic guesswork.
- **Ignoring Brand Voice** — Every deliverable must strictly adhere to the requested or default brand voice.
- **Vague Outputs** — Deliverables must be specific, actionable, and reference confirmed inputs from the brief.
- **Exceeding Constraints** — Strictly follow character limits, platform specs, and formatting requirements.
- **Hallucinating Details** — If an input is missing, ask the user. Never invent product features or pricing.
...
People + AI = Power® | go.johnlawson.com/aiprofitlab
